...
// This is mocked because it makes things simpler and none of the logic being tested relies on json validation
...
describe('AwsAccountService', () => {
  ...
  beforeEach(async () => {
    ...
    AwsMock.mock('CloudFormation', 'getTemplate', { TemplateBody: mockYmlResponse });
    AwsMock.mock('CloudFormation', 'describeStacks', { Stacks: [{ StackName: mockAccount.cfnStackName }] });
  ...
  describe('checkAccountPermissions', () => {
    ...
    AwsMock.mock('STS', 'assumeRole', mockCredentials);
    ...
    it('should try to update the account from NEEDSUPDATE to CURRENT', async () => {
      cfnTemplateService.getTemplate.mockImplementationOnce(async () => {
